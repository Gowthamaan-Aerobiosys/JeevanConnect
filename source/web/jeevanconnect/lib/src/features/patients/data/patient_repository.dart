...
class PatientRepository {
  ...
  Future<bool> addClinicalLog(log, id, addedBy) async {
    final response = await http.post(AppUrls.addClinicalLog,
        body: {
          "amr_id": id,
          "content": log,
          "added_by": addedBy,
  ...
  Future<bool> removeClinicalLog(index, id) async {
    final response = await http.post(AppUrls.removeClinicalLog,
        body: {
          "amr_id": id,
          "index": index,
        },
  ...
  Future<bool> addABGRecord(record) async {
    final response = await http.post(AppUrls.addABGLog,
        body: record, headers: AuthenticationRepository().headers);
    if (response.statusCode == 302) {
      String url = response.headers['location'] ?? " ";
      final getResponse = await http.get(Uri.parse(url));
  ...
  Future<bool> removeABGRecord(indices, id) async {
    final response = await http.post(AppUrls.removeABGLog,
        body: {
          "amr_id": id,
          "indices": indices,
        },
  ...
  Future getABGGraph(recordId) async {
    try {
      final response = await http.get(
        AppUrls.getABGLogGraph(recordId),
      );
      final result = (convert.jsonDecode(response.body))['DATA'];
      return result;
    } catch (exception) {
