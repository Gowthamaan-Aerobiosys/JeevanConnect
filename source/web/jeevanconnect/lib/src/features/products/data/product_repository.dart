...
class ProductRepository {
  ...
  Future<bool> registerProduct({required productData}) async {
    try {
      final response = await http.post(AppUrls.registerProduct,
          body: productData, headers: AuthenticationRepository().headers);
  ...
  Future<PaginatedList<Product>> getWorkspaceProducts(workspaceId) async {
    try {
      final response =
          await http.get(AppUrls.getWorkspaceProducts(workspaceId));
      final result = (convert.jsonDecode(response.body))['DATA'];
